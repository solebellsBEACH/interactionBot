...
export class LinkedinDiscordCommands {
    ...
    register(discord: DiscordClient) {
        discord.setCommandHandlers({
            ...
            [CommandName.UpvotePosts]: async () => {
                ...
                const parsed = this._parseQuantityAndTag(quantityAnswer)
    ...
    private _normalizeAnswer(value: string) {
        return value
            .normalize('NFD')
            .replace(/[\u0300-\u036f]/g, '')
            .trim()
    ...
    private _normalizeArgValue(value?: string) {
        if (!value) return undefined
        return value.replace(/[+_]/g, ' ').trim()
    }

    private _parseArgNumber(value?: string) {
        if (!value) return undefined
    ...
    private _parseSearchArgs(args: string[]) {
        ...
        const parseInline = (rawValue: string, normalizedValue: string, keys: string[]) => {
            for (const key of keys) {
                if (normalizedValue.startsWith(`${key}:`)) {
                    ...
                    return this._normalizeArgValue(rawValue.slice(idx + 1))
                }
                if (normalizedValue.startsWith(`${key}=`)) {
                    const idx = rawValue.indexOf('=')
                    return this._normalizeArgValue(rawValue.slice(idx + 1))
        ...
        const collectValue = (startIndex: number) => {
            ...
            return {
                value: this._normalizeArgValue(tokens.join(' ')),
                endIndex: idx - 1
            }
        }
        ...
        for (let i = 0; i < parts.length; i++) {
            ...
            if (normalized === '--pages' || normalized === '--page' || normalized === '--paginas' || normalized === '--pagina' || normalized === '--max-pages') {
                maxPages = this._parseArgNumber(parts[i + 1])
                i++
                continue
            }
            ...
            if (inlinePages) {
                maxPages = this._parseArgNumber(inlinePages)
                continue
            }
    ...
    private _parseMaxResultsAnswer(value: string | null) {
        if (!value) return undefined
        const normalized = this._normalizeAnswer(value)
    ...
    private _parseLocationAnswer(value: string | null) {
        if (!value) return undefined
        const trimmed = value.trim()
        if (!trimmed) return undefined
        const normalized = this._normalizeAnswer(trimmed)
        if (/^(skip|pular|nao|n)$/.test(normalized)) return undefined
        return trimmed
    }

    private _parseMaxPagesAnswer(value: string | null) {
        if (!value) return undefined
        const normalized = this._normalizeAnswer(value)
        if (!normalized) return undefined
    ...
    private _isStop(value: string) {
        const normalized = this._normalizeAnswer(value)
        if (!normalized) return false
        return /^(parar|stop|sair|cancelar)\b/.test(normalized)
    }

    private _isSkip(value: string) {
        const normalized = this._normalizeAnswer(value)
        if (!normalized) return false
